...
def log_details():
    ...
    def log_execution_date(**kwargs):
        logging.info(f"Execution date is {kwargs['']}")

    @task
    def log_run_id(**kwargs):
        logging.info(f"My run id is {kwargs['']}")
    
    @task
    def log_previous_run(**kwargs):
        logging.info(f"My previous run was on {kwargs['']}")
    @task
    def log_next_run(**kwargs):
        logging.info(f"My next run will be {kwargs['']}")
